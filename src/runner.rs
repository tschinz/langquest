...
impl VerificationResult {
    ...
    pub fn progress_bar(&self, width: usize) -> String {
        let filled = if self.total > 0 {
            (self.score * width as f64).round() as usize
        } else {
            0
        }
        .min(width);
...
fn verify_rust(exercise: &Exercise, rust_cfg: &RustConfig) -> VerificationResult {
    ...
    // --- compile --------------------------------------------------------
    let (compile_bin, compile_args) =
        match build_rust_command(rust_cfg, &exercise.source_path, &test_bin) {
            Ok(b) => b,
            Err(e) => return VerificationResult::zero(e, threshold),
        };

    let compile = Command::new(&compile_bin)
        .args(&compile_args)
        .current_dir(&exercise.dir)
        .output();
    ...
    let compile = match compile {
        ...
        Err(e) => {
            return VerificationResult::zero(
                format!("Failed to run {}: {e}", compile_bin.display()),
                threshold,
            );
        }
    };
    ...
    let run = match run {
        ...
        Err(e) => {
            return VerificationResult::zero(
                format!("Failed to execute test binary: {e}"),
                threshold,
            );
        }
    };
...
fn parse_asm_directives(source: &str) -> AsmDirectives {
    ...
    for line in source.lines() {
        ...
        if let Some(rest) = trimmed
            .strip_prefix("# EXPECT_REG:")
            .or_else(|| trimmed.strip_prefix("; EXPECT_REG:"))
        {
            let rest = rest.trim();
...
pub(crate) fn find_ripes_binary() -> Option<PathBuf> {
    ...
    if let Ok(cwd) = std::env::current_dir() {
        if let Some(bin) = find_bundled_ripes(&cwd) {
            return Some(bin);
        }
    }
...
fn build_ripes_command(
    ripes_cfg: &RipesConfig,
    source_file: &str,
) -> Result<(PathBuf, Vec<String>), String> {
    ...
    } else if program_token.contains(std::path::MAIN_SEPARATOR)
        || program_token.contains('/')
    {
...
fn parse_ripes_registers(json_str: &str) -> Result<HashMap<String, i64>, String> {
    ...
    let json_start = json_str
        .find('{')
        .ok_or_else(|| "no JSON object found in Ripes output".to_string())?;
    ...
    let regs = root
        .get("registers")
        .ok_or_else(|| "no 'registers' key in Ripes JSON output".to_string())?;

    let obj = regs
        .as_object()
        .ok_or_else(|| "'registers' is not a JSON object".to_string())?;
...
fn verify_riscv(exercise: &Exercise, ripes_cfg: &RipesConfig) -> VerificationResult {
    ...
    if !output.status.success() {
        ...
        return VerificationResult::zero(
            format!(
                "Ripes exited with an error:\n{}",
                cap_output(&combined, MAX_OUTPUT_LINES)
            ),
            threshold,
        );
    }
    ...
    let registers = match parse_ripes_registers(&stdout) {
        ...
        Err(e) => {
            return VerificationResult::zero(
                format!(
                    "Could not parse Ripes output: {e}\n\nRaw output (first 50 lines):\n{}",
...
fn verify_python(exercise: &Exercise, python_cfg: &PythonConfig) -> VerificationResult {
    ...
    let (pytest_bin, pytest_args) =
        match build_python_command(python_cfg, &exercise.source_path) {
            Ok(b) => b,
            Err(e) => return VerificationResult::zero(e, threshold),
        };

    // Try pytest first ---------------------------------------------------
    let pytest = Command::new(&pytest_bin)
        .args(&pytest_args)
        .current_dir(&exercise.dir)
        .output();
    ...
    match pytest {
        ...
        Err(e) => VerificationResult::zero(
            format!("Failed to run {}: {e}", pytest_bin.display()),
            threshold,
        ),
    }
}
...
fn verify_python_fallback(exercise: &Exercise, python_bin: &Path) -> VerificationResult {
    ...
    let run = Command::new(python_bin)
        .arg(&exercise.source_path)
        .current_dir(&exercise.dir)
        .output();
    ...
    match run {
        ...
        Err(e) => VerificationResult::zero(
            format!("Failed to run {}: {e}", python_bin.display()),
            threshold,
        ),
    }
}
...
fn verify_markdown(exercise: &Exercise) -> VerificationResult {
    ...
    let solution_data = match &exercise.solution_data {
        ...
        None => {
            return VerificationResult::zero(
                "No solution data found for keyword verification.".to_string(),
                threshold,
            );
        }
    };
    ...
    if keywords.is_empty() {
        return VerificationResult::zero(
            "No keywords defined in solution data.".to_string(),
            threshold,
        );
    }
