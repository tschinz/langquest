...
fn render_exercise_table(
    ...
    let columns = vec![
        Column { header: "ID".to_string(), width: 20 },
        Column { header: "Name".to_string(), width: 30 },
        Column { header: "Language".to_string(), width: 12 },
        Column { header: "Difficulty".to_string(), width: 10 },
        Column { header: "Status".to_string(), width: 12 },
        Column { header: "Topics".to_string(), width: 30 },
    ];

    let rows: Vec<Vec<String>> = exercises
        .iter()
        .map(|&(mi, ei)| {
            let (id, name, lang_name, difficulty, status_text, topics) =
                if let Some(module) = modules.get(mi) {
                    if let Some(ex) = module.exercises.get(ei) {
                        ...
                        let status_str =
                            format!("{} {}", status.symbol(), status.label());
...
fn render_tree_panel(
    ...
    for (mi, module) in modules.iter().enumerate() {
        ...
        for (ei, ex) in module.exercises.iter().enumerate() {
            ...
            lines.push(Line::from(Span::styled(
                format!("  {connector} {} {}", status.symbol(), ex.name),
                style,
            )));
        }
