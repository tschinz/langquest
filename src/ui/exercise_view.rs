...
use crate::ui::markdown::{parse_markdown_with_links, LinkSpan, PendingOsc8};
...
fn render_topbar(frame: &mut Frame, area: Rect, exercise: &Exercise) {
    let spans = vec![
        Span::raw("  "),
        Span::styled(
            exercise.relative_path.clone(),
            Style::default().fg(TOPBAR_PATH_FG),
        ),
    ...
    frame.render_widget(
        Paragraph::new(Line::from(spans)).style(Style::default().bg(TOPBAR_BG)),
        area,
    );
}
...
fn render_content(frame: &mut Frame, area: Rect, params: &ViewParams<'_>) -> Option<PendingOsc8> {
    match params.page {
        ExercisePage::Theory => {
            render_theory(frame, area, params.exercise, params.scroll_offset)
        }
        ExercisePage::Task => {
            render_task(frame, area, params.exercise, params.scroll_offset)
        }
        ExercisePage::Output => {
            render_output(
                frame,
        ...
        ExercisePage::Solution => render_solution(
            frame,
            area,
            params.exercise,
            &params.config_state,
...
fn render_theory(
    ...
    Some(PendingOsc8 { area: inner, scroll: scroll_offset, links })
}
...
fn render_output(
    ...
    let lines = match last_result {
        ...
        None => vec![Line::from(
            "No verification result yet. Save your file to trigger verification.",
        )],
    };
...
fn render_solution(
    ...
    if let Some(ref solution_path) = exercise.solution_source {
        match fs::read_to_string(solution_path) {
            Ok(source) => {
                ...
                lines.extend(crate::ui::markdown::highlight_code_block(
                    &source, token, area.width,
                ));
            }
            Err(e) => {
    ...
    frame.render_widget(paragraph, area);
    Some(PendingOsc8 { area: inner, scroll: scroll_offset, links: solution_links })
}

/// Append the explanation section from `SolutionData` into `lines`.
fn append_explanation(
    lines: &mut Vec<Line<'static>>,
    solution_data: &crate::exercise::SolutionData,
    width: u16,
) -> Vec<LinkSpan> {
    if !lines.is_empty() {
        lines.push(Line::from(""));
