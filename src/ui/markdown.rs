...
fn code_line(spans: Vec<Span<'static>>, line_num: usize, num_width: usize, width: u16) -> Line<'static> {
    ...
    all.push(Span::styled(
        "  │  ",
        Style::default().fg(Color::Rgb(55, 60, 78)).bg(CODE_BG),
    ));
    // Code content.
    all.extend(spans);
            ...
            Tag::Item => {
                ...
                let prefix = if is_ordered {
                    let n = self
                        .item_numbers
                        .last_mut()
                        .expect("item_numbers always in sync with list_stack");
...
mod tests {
    ...
    fn italic_text_is_purple() {
        ...
        let span = lines[0]
            .spans
            .iter()
            .find(|s| s.content.contains("soft"))
            .expect("italic span");
        assert_eq!(span.style.fg, Some(Color::Rgb(188, 120, 255)));
        assert!(span.style.add_modifier.contains(Modifier::ITALIC));
    }
    ...
    fn link_text_is_light_blue() {
        ...
        let span = lines[0]
            .spans
            .iter()
            .find(|s| s.content.contains("Rust book"))
            .expect("link span");
    ...
    fn fenced_code_block_is_highlighted() {
        ...
        let code_line = lines
            .iter()
            .find(|l| text_of(l).contains("let"))
            .expect("a line containing 'let'");
...
mod code_block_layout_tests {
    ...
    fn has_code_bg(l: &Line<'_>) -> bool {
        l.style.bg == Some(CODE_BG)
            || l.spans.iter().any(|s| s.style.bg == Some(CODE_BG))
    }
    ...
    fn blank_separator_after_last_code_line() {
        ...
        assert!(
            last_code + 1 < p.len(),
            "blank separator should follow last code line"
        );
        assert!(
            !p[last_code + 1],
            "line after last code line must be blank (no CODE_BG)"
        );
    }
    ...
    fn list_item_code_block_merged_into_one() {
        ...
        let code_lines: Vec<_> = lines.iter()
        ...
        let texts: Vec<String> = code_lines.iter()
