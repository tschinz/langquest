...
mod discovery {
    ...
    fn discovers_all_modules() {
        ...
        assert!(
            errors.is_empty(),
            "unexpected discovery errors: {errors:?}"
        );
    }
    ...
    fn exercise_has_theory_and_task_paths() {
        ...
        assert!(
            ex.theory_path.is_some(),
            "theory_path should be set for {}",
            ex.id
        );
        assert!(
            ex.theory_path.as_ref().unwrap().exists(),
            "theory_path should point to an existing file"
        );
        assert!(
            ex.task_path.exists(),
            "task_path should point to an existing file"
        );
        assert!(
            ex.source_path.exists(),
            "source_path should point to an existing file"
        );
    }
    ...
    fn exercise_has_solution_data() {
        ...
        assert!(
            ex.solution_data.is_some(),
            "solution_data should be loaded for {}",
            ex.id
        );
    ...
    fn relative_path_is_module_slash_exercise() {
        ...
        assert!(
            ex.relative_path.contains('/'),
            "relative_path should contain a slash: {}",
            ex.relative_path
        );
        assert!(
...
mod config {
    ...
    fn save_and_load_roundtrip() {
        ...
        assert_eq!(
            loaded.current_exercise.as_deref(),
            Some("01-basics/01-hello")
        );
...
mod runner {
    ...
    fn verify_markdown_exercise_from_sample_repo() {
        ...
        let concept = modules
            .iter()
            .find(|m| m.name == "08-concepts")
            .and_then(|m| m.exercises.first());
...
mod end_to_end {
    ...
    fn discover_then_persist_initial_config() {
        ...
        for module in &modules {
            for exercise in &module.exercises {
                let _ = cfg
                    .exercises
                    .entry(exercise.relative_path.clone())
                    .or_default();
            }
        }
    ...
    fn simulated_exercise_completion_flow() {
        ...
        assert_eq!(
            cfg.current_exercise.as_deref(),
            Some("01-rust/02-variables")
        );
    }
    ...
    fn reset_preserves_only_first_exercise_pointer() {
        ...
        assert!(
            cfg.current_exercise.is_some(),
            "reset should set current to first exercise"
        );
    }
}
